...
namespace TheBugTracker.Services
{
    public class BTTicketService : IBTTicketService
    {
            ...
            try
            {
                if (ticket != null)
                {
                    try
                    {
                        ...
                        ticket.TicketStatusId = (await LookupTicketStatusIdAsync("Develpment")).Value;
                        await _context.SaveChangesAsync();
                    }
                    catch (Exception)
                    {
        ...
        {
            //get status id using statusname
            //try catch: - access ticket collection in projects
            //           - with companyId & status id
            ...
            try
            {
                List<Ticket> tickets = await _context.Projects
                                                     .Where(p => p.CompanyId == companyId)
                                                     .SelectMany(p => p.Tickets)    //select many tickets in many projects
                                                        .Include(t => t.Attachments)
                                                        .Include(t => t.Comments)
                                                        .Include(t => t.DeveloperUser)
                                                        .Include(t => t.OwnerUser)
        ...
        {
            //get typeid using typename
            //try catch: - access ticket collection in projects
            //           - with companyid & typeid
        ...
        {
            //get all company's tickets
            //return a list of archived tickets
            try
            {
